...
export function decideRetry(options: RetryPolicyOptions & { error: unknown }): RetryDecision {
  ...
  const retryable = /(tests?\s+(failed|failing)|assert(?:ion)?\s+(failed|error)|verification\s+failed|diff\s+(failed|check)|compile(?:r)?\s+(failed|error)|syntax\s+error|type(?:script)?\s+error)/i.test(message);
  return {
