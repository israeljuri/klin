...
export async function runPipeline(options: PipelineOptions): Promise<TaskResult> {
  ...
  if (options.budgetUsd !== undefined && options.inputRatePerMillion !== undefined && options.outputRatePerMillion !== undefined) {
    // Preflight only uses the input estimate. Output is intentionally accounted for after the API call,
    // because the API's actual output_tokens are authoritative.
  ...
  let verification;
  try {
    verification = await reconcileAndVerify(modelResult.edits, options.root, options.task.test_command ?? 'node --test');
  } catch (error) {
    ...
    await appendLedger(options.root, makeLedgerEntry(options, 'failed', cost, usage, [], error instanceof Error ? error.message : String(error)));
    throw error;
  }
  ...
  const cost = usage && options.inputRatePerMillion !== undefined && options.outputRatePerMillion !== undefined
    ? estimateCostUsd(usage, options.inputRatePerMillion, options.outputRatePerMillion) : 0;
  if (options.budgetUsd !== undefined) assertWithinBudget({ limit_usd: options.budgetUsd, spent_usd: spent }, cost);
  await appendLedger(options.root, makeLedgerEntry(options, 'completed', cost, usage, verification.result.changed_files));
  ...
  return {
    ...
    model_usage: usage,
    response_id: modelResult.response_id,
    changed_files: verification.result.changed_files,
  };
}

function makeLedgerEntry(options: PipelineOptions, status: LedgerEntry['status'], cost: number, usage: TaskResult['model_usage'], changedFiles: string[], error?: string): LedgerEntry {
  return {
    task_id: options.task.id,
    status,
...
export function createLiveModelFromEnv(): { model: Model; modelName: string } {
  ...
  return {
    modelName,
    model: createResponsesAdapter({ model: modelName, endpoint, apiKey: process.env.META_API_KEY, dryRun: false }),
  };
}
