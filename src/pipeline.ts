...
export async function runPipeline(options: PipelineOptions): Promise<TaskResult> {
  ...
  if (!options.dryRun && options.budgetUsd !== undefined && hasRates) {
    if (options.budgetCoordinator) reservation = await options.budgetCoordinator.reserve(estimatedInputCost);
    else assertWithinBudget({ limit_usd: options.budgetUsd, spent_usd: spent }, estimatedInputCost);
  }
  ...
  if (options.dryRun) return { task_id: options.task.id, status: 'pending-model', context, model_usage: modelResult.usage, response_id: modelResult.response_id };
  ...
  try {
    ...
    const cost = usage && hasRates ? estimateCostUsd(usage, options.inputRatePerMillion!, options.outputRatePerMillion!) : 0;
...
function makeLedgerEntry(options: PipelineOptions, status: LedgerEntry['status'], cost: number, usage: TaskResult['model_usage'], changedFiles: string[], contextCharacters: number, error?: string): LedgerEntry {
  return { task_id: options.task.id, status, model: options.modelName, attempts: 1, context_characters: contextCharacters, input_tokens: usage?.input_tokens ?? 0, cached_tokens: usage?.cached_tokens ?? 0, output_tokens: usage?.output_tokens ?? 0, reasoning_tokens: usage?.reasoning_tokens ?? 0, total_tokens: usage?.total_tokens ?? 0, estimated_cost_usd: cost, changed_files: changedFiles, ...(error ? { error } : {}) };
}
