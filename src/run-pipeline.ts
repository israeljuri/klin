...
if (!dryRun && process.env.KLIN_LIVE !== '1') {
  console.error('Refusing a live model request. Use --dry-run for zero-cost execution or set KLIN_LIVE=1 explicitly.');
  process.exit(2);
}
...
console.log(`Mode: ${dryRun ? 'DRY RUN (no API call)' : 'LIVE MODEL REQUEST'}`);
...
console.log(JSON.stringify({
  ...
  context_characters: result.context.characters,
