...
export function buildModelPrompt(request: Omit<ModelRequest, 'prompt'>): string {
  ...
  return [
    ...
    '', 'TASK', request.task.description, '', 'OUTPUT CONTRACT',
...
export async function executeTask(task: Task, root: string, model: Model): Promise<TaskResult> {
  ...
  try {
    ...
    if (!verification.result.success) {
      return { task_id: task.id, status: 'failed', context, model_usage: modelResult.usage, response_id: modelResult.response_id, error: verification.result.error };
    }
