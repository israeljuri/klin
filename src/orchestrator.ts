...
export function buildModelPrompt(request: ModelRequest): string {
  const context = request.context.files
    .map(({ path, content }) => `===== ${path} =====\n${content}`)
    .join('\n\n');

  return [
    'You are Klin\'s implementation model.',
    'Implement the task using only the supplied repository context.',
    '',
    'TASK',
    ...
    'Each old_text must match exactly one existing location.',
    'Do not return Git diffs, blob hashes, line numbers, markdown, or explanations.',
    '',
    'REPOSITORY CONTEXT',
    context,
  ].join('\n');
...
export async function executeTask(task: Task, root: string, model: Model): Promise<TaskResult> {
  ...
  const modelResult = await model({ task, context });

  const verification = await reconcileAndVerify(
    modelResult.edits,
    root,
    task.test_command ?? 'node --test',
  );
